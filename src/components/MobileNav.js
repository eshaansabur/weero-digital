...
const MobileNav = () => {
  ...
  return <nav className='text-primary xl:hidden'>
    {/* nav open button */}
    <div onClick={()=>setOpenMenu(true)} className='text-3xl cursor-pointer text-secondary'>
    ...
    className='bg-white shadow-2xl w-full absolute top-0 right-0 max-w-xs h-screen z-50'>
      {/* icon */}
      <div onClick={()=>setOpenMenu(false)} className='text-4xl absolute z-50 left-4 top-14 text-primary cursor-pointer'><IoMdClose></IoMdClose></div>
      {/* menu list */}
